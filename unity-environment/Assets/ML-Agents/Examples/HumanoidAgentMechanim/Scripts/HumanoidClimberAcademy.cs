...
public class HumanoidClimberAcademy : Academy
{
    ...
    public class HumanoidBodyState
    {
        public HumanoidBodyState(int boneCount, int splineInitCount)
        {
            ...
            obstacleHoldPoses = new Vector3[4];
            obstacleHoldScale = new Vector3[4];
            obstacleHoldRots = new Quaternion[4];
        ...
        public float[] currentTargetAngles = new float[30];
        ...
        public Vector3[] obstacleHoldPoses;
        public Vector3[] obstacleHoldScale;
        public Quaternion[] obstacleHoldRots;
    ...
    public class ContextManager
    {
        ...
        public void SetHoldPosition(int i, Vector3 p)
        {
            if (i < 0 || i >= _holdsPositions.Count)
                return;
            _holdsPositions[i] = p;
    ...
    public class GraphNode
    {
        ...
        int AddAction(int limb_id, int hold_id)
        {
            ...
            if (!IsValidTransition(holdIds, nHoldIds))
            {
                return -1;
            }
    ...
    public class Tools
    {
        ...
        {
            string[] s_vals = s.Split(new char[] {','});

            return new Vector3(float.Parse(s_vals[0]), float.Parse(s_vals[1]), float.Parse(s_vals[2]));
        }
    ...
    public class TestPerformanceNode
    {
        public TestPerformanceNode()
        {
            fromToStates = new List<int>[MaxNumTrialPerPerformanceNodeEdge];
            for (int i = 0; i < MaxNumTrialPerPerformanceNodeEdge; i++)
            {
                fromToStates[i] = new List<int>();
            }
        }
        public List<int> initStates = new List<int>();
        public int[] targetHoldIds = { -1, -1, -1, -1 };
        public int[] currentHoldIds = { -1, -1, -1, -1 };
        public int numOfTrial = 0;

        public List<int>[] fromToStates; // keep the synthesized states from parent node performance to the current (this) node
        public List<int> fromIndexInitState = new List<int>(); // determine each init state and 'fromToStates' initiated from which init state in the path performance
    }
    ...
    {
        RandomTraining = 0, // alwayse starts from init, do random transition until max_task_in_episode is done
        ...
        UniformTraining = 6,
        PSITargetTraining = 7,
        UniformAroundStateTraining = 8,
    };
    
    public bool flag_test_performance = false; // set this in editor to true in case of reporting statistics on the designed routes!
    [HideInInspector] public bool IsSearchDone = false;
    public bool flag_show_video_when_path_found = false;
    [HideInInspector] public bool useSameRewarding = false;
    [HideInInspector] public const bool MultiStepAction = false;
    ...
    [HideInInspector] public const int max_count_limb_movement = 2;
    [HideInInspector] public const float minSuccessRate = 0.1f;

    [HideInInspector] public const int max_task_in_episode = current_training_type == TrainingType.RandomTraining ? 10 : 0;
    [HideInInspector] public const int max_num_fails_in_episode = current_training_type == TrainingType.PSITargetTraining ? 2 : 0;
    [HideInInspector] const string base_file_name = MultiStepAction ? "sample_uniform_wall_MultiStep_" : "sample_uniform_wall_oneStep_";
    ...
    [HideInInspector] public List<HumanoidClimberAgent> _ControlRigs = new List<HumanoidClimberAgent>();
    ...
    int[] bodyTransitionTrialCount = { 0, 0, 0, 0 };
    int[] bodyTransitionSuccessCount = { 0, 0, 0, 0 };

    [HideInInspector] public int loggedStep = 0;
    [HideInInspector] public int numUpdatedPolicy = 0;

    [HideInInspector] public float[] limitedXArea;
    [HideInInspector] public float[] limitedYArea;
    const int visited_pos_size_x = 8 * 25;
    const int visited_pos_size_y = (int)(9.5f * 25);
    ...
    int count_to_end = 0;
    int samplePoseSiteCounter = 0;
    int numOfSamplesToReport = 0;
    int poseSiteLoggedStep = 0;

    // testing performance
    [HideInInspector] public List<TestPerformanceNode> testPerformanceNodes = new List<TestPerformanceNode>();
    const int MaxNumTrialPerPerformanceNodeEdge = 100;
    public void CompleteTestPerformanceNode(List<int> state_ids, int current_step, int _trajectoryIdx, Vector2Int fromInitIndexPerformanceNodeIds, bool isItSucceeded)
    {
        int _fromNodeId = fromInitIndexPerformanceNodeIds[0];
        int _initStateId = fromInitIndexPerformanceNodeIds[1];
        int _cTestNodeId = _fromNodeId + 1;
        if (!flag_test_performance)
            return;
        if (_cTestNodeId < 0 || _cTestNodeId >= testPerformanceNodes.Count)
            return;
        if (_trajectoryIdx < 0 || _trajectoryIdx >= _ControlRigs.Count)
            return;
        ...
        if (isItSucceeded)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                if (_ControlRigs[_trajectoryIdx].GetCurrentHoldID(i) != testPerformanceNodes[_cTestNodeId].currentHoldIds[i])
                {
                    flag_save = false;
                }
            }
            ...
            {
                int fromToIndex = -1;
                if (testPerformanceNodes[_cTestNodeId].initStates.Count < MaxNumTrialPerPerformanceNodeEdge)
                {
                    int save_state_id = GetNextFreeState(_ControlRigs[_trajectoryIdx].jdController.getBonesCount(), _ControlRigs[_trajectoryIdx].spline.Count);
                    _ControlRigs[_trajectoryIdx].SaveState(save_state_id);
                    testPerformanceNodes[_cTestNodeId].initStates.Add(save_state_id);

                    fromToIndex = testPerformanceNodes[_cTestNodeId].initStates.Count - 1;
                }
                
                if (_fromNodeId > -1)
                {
                    testPerformanceNodes[_cTestNodeId].fromIndexInitState.Add(_initStateId);
                    if (fromToIndex > -1)
                    {
                        for (int i = 0; i < current_step && i < state_ids.Count; i++)
                        {
                            testPerformanceNodes[_cTestNodeId].fromToStates[fromToIndex].Add(CopyState(state_ids[i]));
                        }
                    }
                }
            }
    ...
    public void GetNextPlannedTargetHoldIds(ref Vector2Int fromInitIndexPerformanceNodeIds, ref int[] planned_hold_ids)
    {
        if (!flag_test_performance)
        {
            return;
        }
        if (testPerformanceNodes == null || planned_hold_ids == null)
            return;

        int cPerformanceNodeId = fromInitIndexPerformanceNodeIds[0];
        if (cPerformanceNodeId >= testPerformanceNodes.Count)
        {
            return;
        }
        
        if (cPerformanceNodeId < 0)
        {
            cPerformanceNodeId = 0;
            fromInitIndexPerformanceNodeIds[0] = 0;
            fromInitIndexPerformanceNodeIds[1] = 0;
        }
        
        for (int b = 0; b < 4; b++)
        {
            planned_hold_ids[b] = testPerformanceNodes[cPerformanceNodeId].targetHoldIds[b];
        }
    }

    private void myExit()
    {
        if (!flag_show_video_when_path_found)
        {
#if UNITY_EDITOR
    ...
    public bool IsHandSampleNeeded()
    {
        return (bodyTransitionTrialCount[0] + bodyTransitionTrialCount[1]) > (bodyTransitionTrialCount[2] + bodyTransitionTrialCount[3]);
    }
    ...
    private void WritePoseRewardMakeZero()
    {
        if (numOfSamplesToReport <= 0)
        {
            numOfSamplesToReport = 0;
            return;
        }
        // writeToFile
        int index = 0;

        while (File.Exists(base_file_name + worker_id.ToString() + "_" + "pos_reward_b0_" + index.ToString() + ".txt"))
        {
            index++;
        }
        ...
        for (int b = 0; b < 4; b++)
        {
            StreamWriter statistics_reward_Writer = new StreamWriter(base_file_name + worker_id.ToString() + "_" + "pos_reward_b" + b.ToString() + "_" + index.ToString() + ".txt");
            for (int row = 0; row < visited_pos_size_y; row++)
            {
                string line_string = "";
                for (int col = 0; col < visited_pos_size_x; col++)
                {
                    float v = visited_pos_r[col][row][b] / (visited_pos_s[col][row][b] + 1e-6f);
                    visited_pos_r[col][row][b] = 0;
                    visited_pos_s[col][row][b] = 0;
                    if (col == visited_pos_size_x - 1)
                    {
                        line_string += (v).ToString();
                    }
                    else
                    {
                        line_string += (v).ToString() + ",";
    ...
    public void AddSitePos(float x, float y, float r, bool isSucceed, int[] bodyIndex)
    {
        if (!flag_test_performance || HumanoidClimberAgent.useAgentInterface)
            return;

        int maxVal = (testPerformanceNodes.Count - 1) * MaxNumTrialPerPerformanceNodeEdge;
        if (samplePoseSiteCounter < maxVal)
        {
            //int index_x = (int)((x + Mathf.Abs(limitedXArea[0])) * (visited_pos_size_x / 8.0f));
            //int index_y = (int)((y + Mathf.Abs(limitedYArea[0])) * (visited_pos_size_y / 9.5f));
            ...
            for (int b = 0; b < 4; b++)
            {
                if (bodyIndex[b] >= 0)
                {
                    //visited_pos_c[index_x][index_y][bodyIndex[b]]++;
                    //if (isSucceed)
                    //{
                    //    visited_pos_r[index_x][index_y][bodyIndex[b]] += r;
                    //    visited_pos_s[index_x][index_y][bodyIndex[b]]++;
                    //    numOfSamplesToReport++;
                    //}
                }
            }
            samplePoseSiteCounter++;
        }
        else if (samplePoseSiteCounter == maxVal)
        {
            samplePoseSiteCounter = maxVal + 10;
            //WritePoseRewardMakeZero();
            WriteStatistics(1000);
    ...
    public void AddTransitionReward(float accumulative_reward, int episode_length, int num_spline_used, bool isTrajectoryForked, bool succeeded_transition, int[] bodyIndex)
    {
        if (!isTrajectoryForked)
        {
            ...
            for (int b = 0; b < 4; b++)
            {
                if (bodyIndex[b] >= 0)
                {
                    bodyTransitionTrialCount[bodyIndex[b]]++;
                    if (succeeded_transition)
                    {
                        bodyTransitionSuccessCount[bodyIndex[b]]++;
                    }
    ...
    int CopyState(int _fromStateId, int _toStateId)
    {
        ...
        for (int b = 0; b < 4; b++)
        {
            _cState.obstacleHoldPoses[b] = _fState.obstacleHoldPoses[b];
            _cState.obstacleHoldScale[b] = _fState.obstacleHoldScale[b];
            _cState.obstacleHoldRots[b] = _fState.obstacleHoldRots[b];
        ...
        for (int j = 0; j < _fState.currentTargetAngles.Length; j++)
        {
            _cState.currentTargetAngles[j] = _fState.currentTargetAngles[j];
        }
    ...
    void ResetTrajecotriesIfNecessary()
    {
        ...
        if (File.Exists(update_path))
        {
            ...
            int divideVal = (int)(loggedStep / 1e5);
            if (divideVal != poseSiteLoggedStep)
            {
                poseSiteLoggedStep = divideVal;
                WritePoseRewardMakeZero();
                samplePoseSiteCounter = 0;
            }
        }
    }
    ...
    private void OpenStatisticsFile()
    {
        if (streamWriter == null && isWorkerIDSet)
        {
            ...
            if (flag_test_performance && !HumanoidClimberAgent.useAgentInterface)
            {
                worker_id_base_file_name += _RouteNum.ToString() + "_";
            }
    ...
    private void WriteStatistics(int cStep)
    {
        ...
        if (flag_test_performance)
        {
            int max_count_trial = 0;
            for (int i = 0; i < testPerformanceNodes.Count; i++)
            {
                if (Tools.GetNumConnectedLimbs(testPerformanceNodes[i].targetHoldIds) > 0)
                    max_count_trial += Mathf.Max(100, testPerformanceNodes[i].numOfTrial);
            }
            
            int trial_transitions = 0;
            int success_transtiosns = 0;
            for (int b = 0; b < 4; b ++)
            {
                trial_transitions += bodyTransitionTrialCount[b];
                success_transtiosns += bodyTransitionSuccessCount[b];
            }
            string val = nonForkedTransition_accumulative_reward.ToString() + ","
                + max_count_trial.ToString() + ","
                + nonForkedTransition_EpisodeLength.ToString() + ","
                + nonForkedTransitionCounter.ToString() + ","
                + trial_transitions.ToString() + ","
                + max_count_trial.ToString() + ","
                + success_transtiosns.ToString() + ","
                + max_count_trial.ToString();
            ...
            for (int b = 0; b < 4; b++)
            {
                bodyTransitionSuccessCount[b] = 0;
                bodyTransitionTrialCount[b] = 0;
            }
        ...
        if (nonForkedEpisodeCounter > 0 || nonForkedTransitionCounter > 0)
        {
            string val = cStep.ToString() + ","
                ...
                + bodyTransitionTrialCount[0].ToString() + ","
                + bodyTransitionTrialCount[1].ToString() + ","
                + bodyTransitionTrialCount[2].ToString() + ","
                + bodyTransitionTrialCount[3].ToString() + ","
                + bodyTransitionSuccessCount[0].ToString() + ","
                + bodyTransitionSuccessCount[1].ToString() + ","
                + bodyTransitionSuccessCount[2].ToString() + ","
                + bodyTransitionSuccessCount[3].ToString();
    ...
    {
        if (_trajectoryIdx > -1 && _trajectoryIdx < _ControlRigs.Count
            && graphId >= 0 && graphId < graphNodes.Count)
        {
            for (int i = 0; i < 4; i++)
            {
            ...
            {
                graphNodes[graphId].humanoidStateIds[_trajectoryIdx] = GetNextFreeState(_ControlRigs[_trajectoryIdx].jdController.getBonesCount(), _ControlRigs[_trajectoryIdx].spline.Count);
            }
            ...
            if (flag_update_state)
            {
                ...
                UpdateHoldIdsAroundStanceNodeGivenHipLocation(graphNodes[graphId].nodeIdx, savedStates[savedNodeIdx].pos[0]); // 1 is for chest position
    ...
    public Vector2Int GetRandomState(int _trajectoryIdx, ref int[] planned_target_ids, ref Vector2Int _fromInitIndexPerformanceNodeIds)
    {
        _fromInitIndexPerformanceNodeIds[0] = -1;
        _fromInitIndexPerformanceNodeIds[1] = -1;
        if (flag_test_performance && !HumanoidClimberAgent.useAgentInterface)
        {
            if (testPerformanceNodes == null)
                return new Vector2Int(0, 0);

            List<int> valid_nodes = new List<int>();
            for (int i = 0; i < testPerformanceNodes.Count; i++)
            {
                if (testPerformanceNodes[i].initStates.Count > 0 && 
                    Tools.GetNumConnectedLimbs(testPerformanceNodes[i].targetHoldIds) > 0 &&
                    testPerformanceNodes[i].numOfTrial < MaxNumTrialPerPerformanceNodeEdge)
                    valid_nodes.Add(i);
            }

            if (valid_nodes.Count == 0)
            {
                WriteStatistics(1000);
                myExit();
                IsSearchDone = true;
            ...
            int rnd_node_index = valid_nodes[Random.Range(0, valid_nodes.Count)];
            int rnd_state_idx = Random.Range(0, testPerformanceNodes[rnd_node_index].initStates.Count);

            int load_state_id = testPerformanceNodes[rnd_node_index].initStates[rnd_state_idx];
            testPerformanceNodes[rnd_node_index].numOfTrial++;
            for (int b = 0; b < 4; b++)
            {
                planned_target_ids[b] = testPerformanceNodes[rnd_node_index].targetHoldIds[b];
            }
            _fromInitIndexPerformanceNodeIds[0] = rnd_node_index;
            _fromInitIndexPerformanceNodeIds[1] = rnd_state_idx;

            return new Vector2Int(load_state_id, 0);
        }
        ...
        if (current_training_type == TrainingType.UniformTraining || current_training_type == TrainingType.UniformAroundStateTraining)
        {
    ...
    public void UpdateInitHoldPosition(int hold_id, float cDisBtwHolds)
    {
        if (mContext == null)
        {
            mContext = new ContextManager(instantObject);
        ...
        Vector3 p = new Vector3((-1.5f + col) * cDisBtwHolds, -4.0f + row * cDisBtwHolds, mContext.HoldPosition(hold_id).z);

        mContext.SetHoldPosition(hold_id, p);
    }
    ...
    public void CompleteNode(int r_idx_node, int plan_count_limb_move)
    {
        ...
        do
        {
            ...
            counter++;
        } while (graphNodes[r_idx_node].possible_actions[plan_count_limb_move].Count == 0 && counter < 100);

        return;
    }

    public void GetRandomPositionAroundAgent(Vector3 hipLocation, int[] cHoldIDs, int[] target_hold_ids, Transform[] holdPoses)
    {
        float step_limit_growing = 1.0f; // 0.5f * (loggedStep / (10 * 1e6f)) + 0.5f;
        float max_r = 4.0f;
        // sample new hip position
        int sample_counter = 1;
        Vector3 nHipLocation = HumanoidClimberAgent.localAgentScaling * hipLocation + new Vector3(0.01398927f, -6.0f, -1.5f);

        for (int b = 0; b < 4; b++)
        {
            if (cHoldIDs[b] >= 0)
            { 
                Vector3 nPos = holdPoses[cHoldIDs[b]].transform.localPosition
                    + Random.Range(0.1f, max_r) * (Quaternion.Euler(0, 0, Random.Range(0.0f, 360.0f)) * Vector3.right);

                nPos = GetRestrictedHoldPos(nPos);

                nHipLocation += nPos;
                sample_counter++;
            }
        }
        nHipLocation /= (float)(sample_counter + 1e-6f);

        for (int b = 0; b < 4; b++)
        {
            if (cHoldIDs[b] >= 0)
            {
                Vector3 nDir = holdPoses[cHoldIDs[b]].transform.localPosition - nHipLocation;

                if (nDir.magnitude > max_r)
                {
                    nHipLocation = holdPoses[cHoldIDs[b]].transform.localPosition + nDir.normalized * max_r;
                }
            }
        }
        ...
        bool isHandConnected = cHoldIDs[2] > -1 || cHoldIDs[3] > -1;
        List<int> val = new List<int>(cHoldIDs);
        for (int b = 0; b < 4; b++)
        {
            if (target_hold_ids[b] != cHoldIDs[b] && target_hold_ids[b] >= 0 && !val.Contains(target_hold_ids[b]))
            {
                switch (b)
                {
                    case 0:
                        minTheta = -0.75f * Mathf.PI - 0.5f * Mathf.PI * step_limit_growing;
                        maxTheta = -0.75f * Mathf.PI + 0.5f * Mathf.PI * step_limit_growing;
                        break;
                    case 1:
                        minTheta = -0.25f * Mathf.PI - 0.5f * Mathf.PI * step_limit_growing;
                        maxTheta = -0.25f * Mathf.PI + 0.5f * Mathf.PI * step_limit_growing;
                        break;
                    case 2:
                        minTheta = 0.75f * Mathf.PI - 0.5f * Mathf.PI * step_limit_growing;
                        maxTheta = 0.75f * Mathf.PI + 0.5f * Mathf.PI * step_limit_growing;
                ...
                {
                    minTheta = 0;
                    maxTheta = 2f * Mathf.PI;
                }
                float theta = Random.Range(minTheta, maxTheta) * Mathf.Rad2Deg;

                Vector3 nPos = nHipLocation + Random.Range(0.1f, max_r) * (Quaternion.Euler(0, 0, theta) * Vector3.right);

                nPos = GetRestrictedHoldPos(nPos);

                holdPoses[target_hold_ids[b]].transform.localPosition = nPos;
            }
        }
    ...
    public bool HasValidDistance(Vector3 hipLocation, int[] target_hold_ids, Transform[] holdPoses)
    {
        if (flag_test_performance)
            return true;

        float max_r = 4.0f;
        //float step_limit_growing = 0.5f * (loggedStep / (10 * 1e6f)) + 0.5f;
        float maxDeltaTheta = Mathf.PI;
        ...
        for (int b = 0; b < 4; b++)
        {
            if (target_hold_ids[b] >= 0)
            {
                Vector3 nPos = holdPoses[target_hold_ids[b]].transform.localPosition
                    + Random.Range(0.1f, max_r) * (Quaternion.Euler(0, 0, Random.Range(0.0f, 360.0f)) * Vector3.right);
        ...
        Vector3 defalutDir = Vector3.right;
        for (int b = 0; b < 4; b++)
        {
            if (target_hold_ids[b] >= 0)
            {
                ...
                {
                    case 0:
                        defalutDir = Quaternion.Euler(0, 0, -0.75f * Mathf.PI * Mathf.Rad2Deg) * Vector3.right;
                        break;
                    case 1:
                        defalutDir = Quaternion.Euler(0, 0, -0.25f * Mathf.PI * Mathf.Rad2Deg) * Vector3.right;
                        break;
                    case 2:
                        defalutDir = Quaternion.Euler(0, 0, 0.75f * Mathf.PI * Mathf.Rad2Deg) * Vector3.right;
                        break;
                ...
                Vector3 cDir = holdPoses[target_hold_ids[b]].transform.localPosition - nHipLocation;
                float dis = cDir.magnitude;
                cDir[2] = 0.0f;
                
                float dTheta = Mathf.Acos(Vector3.Dot(cDir.normalized, defalutDir.normalized));

                if (dTheta > maxDeltaTheta || dis > max_r)
                {
                    return false;
                }
            }
    ...
    public Vector3 GetRestrictedHoldPos(Vector3 nPos)
    {
        float defaultZ = 3.875f;
        nPos.z = defaultZ;
        if (nPos.x < limitedXArea[0])
            nPos.x = limitedXArea[0];
        if (nPos.x > limitedXArea[1])
            nPos.x = limitedXArea[1];

        if (nPos.y < limitedYArea[0])
            nPos.y = limitedYArea[0];
        if (nPos.y > limitedYArea[1])
            nPos.y = limitedYArea[1];
    ...
    void UpdateHoldIdsAroundStanceNodeGivenHipLocation(int cNodeId, Vector3 hipLocation)
    {
        ...
        hipLocation = HumanoidClimberAgent.localAgentScaling * hipLocation + new Vector3(0.01398927f, -6.0f, -1.5f);

        float step_limit_growing = 1.0f;// 0.5f * (loggedStep / (10 * 1e6f)) + 0.5f;
        float radius = 1.3334f * HumanoidClimberAgent.localAgentScaling * step_limit_growing;
        ...
        for (int h = 0; h < mContext.NumHolds(); h++)
        {
            ...
            if (dir.magnitude < radius)
            {
                ...
                if (theta >= (0.25f * Mathf.PI - 0.5f * Mathf.PI * step_limit_growing) && theta <= (0.25f * Mathf.PI + 0.5f * Mathf.PI * step_limit_growing))
                {
                    graphNodes[cNodeId].UpdateHoldIdsForLimb(3, h);
                }
                if ((theta >= (0.75f * Mathf.PI - 0.5f * Mathf.PI * step_limit_growing) && theta <= 0.75f * Mathf.PI + 0.5f * Mathf.PI * step_limit_growing)
                    || (theta <= (-1.25f * Mathf.PI + 0.5f * Mathf.PI * step_limit_growing) && (0.75f * Mathf.PI + 0.5f * Mathf.PI * step_limit_growing > Mathf.PI)))
                {
    ...
    void UpdateHoldIdsAroundStanceNodeGivenRefPoint(Vector2Int ref_point, int cNodeId, int limb_id)
    {
        if (ref_point[0] == -1)
        {
            ...
            int hold_id = limb_to_hold + 1 + 4 * bias;
    ...
    public static bool IsValidStance(int[] curHoldIds)
    {
        ...
        if (hand_hand_dis >= 2.51f || leg_leg_dis >= 3.01f)
        {
            return false;
        }
    ...
    public override void InitializeAcademy()
    {
        ...
        useSameRewarding = flag_test_performance;

        limitedXArea = new float[]{ -4.0f, 4.0f };
        limitedYArea = new float[]{ -5.5f, 4.0f };
        for (int i = 0; i < visited_pos_size_x; i++)
        {
            ...
            for (int j = 0; j < visited_pos_size_y; j++)
            {
                visited_pos_r[i][j] = new float[4];
                visited_pos_c[i][j] = new int[4];
                visited_pos_s[i][j] = new int[4];
            }
        }
        ...
        if (!HumanoidClimberAgent.useAgentInterface)
        {
            StreamReader mRouteNumFile = new StreamReader("ClimberRouteInfo\\RouteNum.txt");
            _RouteNum = int.Parse(mRouteNumFile.ReadLine());
            mRouteNumFile.Close();
            StreamReader userDesiredStanceFile = new StreamReader("ClimberRouteInfo\\RouteStance" + _RouteNum.ToString() + ".txt");

            int c = 0;
            while (!userDesiredStanceFile.EndOfStream)
            {
                string s = userDesiredStanceFile.ReadLine();
                int[] holdIds = Tools.StringToIntArr(s);
                TestPerformanceNode nNode = new TestPerformanceNode();
                for (int h = 0; h < 4; h++)
                {
                    nNode.currentHoldIds[h] = holdIds[h];
                    if (c > 0)
                    {
                        testPerformanceNodes[c - 1].targetHoldIds[h] = holdIds[h];
                    }
                }
                testPerformanceNodes.Add(nNode);
                c++;
            }
